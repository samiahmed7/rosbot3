...
| `smooth.py` | Standalone script (not a ROS node — no `rclpy`). Reads `config/slam_trajectory.csv`, splits it into segments at direction-reversal points (dot product of consecutive direction vectors < -0.5), fits a B-spline per segment (`scipy.interpolate.splprep`), resamples to uniform 5 cm spacing, and writes `config/smoothed_trajectory.csv`. **Must be run from the repo root** (uses relative paths). Requires `pandas`, `numpy`, `scipy`. |
